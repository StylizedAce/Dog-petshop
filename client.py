...
def register_user():
    ...
    if response.status_code == 201:
        ...
        )  # Should throw errorcode in realscale application
    ...
    if response.status_code == 200:
        ...
        for dog in data["dogs"]:
            if dog["status"] == "AVAILABLE":
                print(
                    f"{dog['id']}. {dog['breed']} - {dog['name']} - {dog['price']} gems"
                )
...
def buy_dog():
    while True:
        try:
            dog_id = input(
                "Enter the ID of the dog you want to buy, or write exit to leave: "
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    "Error:",
                    response.json().get(
                        "error", "Unknown error or insufficient balance"
                    ),
                )
