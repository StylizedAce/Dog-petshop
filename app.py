...
def save_dog_info(dog_id, dogs_filename="DB.txt"):
    ...
    with open(dogs_filename, "w") as file:
        for dog in dogs:
            if dog["id"] == dog_id:
                file.write(
                    f"{dog['id']},{dog['breed']},{dog['name']},{dog['price']},SOLD to {session['username']}\n"
                )
                ...
                file.write(
                    f"{dog['id']},{dog['breed']},{dog['name']},{dog['price']},{dog['status']}\n"
                )


def update_dog_data(
    dogs_filename="DB.txt",
):  # This function is here so the list updates realtime and reflects changes to the data in-memory as well
    global dogs
    dogs = load_dogs_from_file(dogs_filename)
...
@app.route(
    "/"
)  # Base endpoint simply returns a list of dogs and the user in session's gem-balance
def home():
    if "username" in session:
        username = session["username"]
        user_balance = users[username]["gems"]
        return jsonify({"gems": user_balance, "dogs": dogs})
...
def login():
    ...
    if user and user["password"] == password:
        session["username"] = (
            username  # Saving the user to session so its easy to extract their name. I could save the whole user object but its not yet necessary
        )
...
def logout():
    session.pop(
        "username", None
    )  # Removes user from session thus taking you back to startscreen
...
def buy_dog(dog_id):
    ...
    dog = next(
        (d for d in dogs if d["id"] == dog_id and d["status"] == "AVAILABLE"), None
    )
    if dog:
...
def reset_shop():
    dogs = load_dogs_from_file(
        "DB.txt"
    )  # Preload dogs from file so we dont truncate the DB
    ...
    for (
        ...
        users
    ):  # This part does not need to preload users as the save method does that already
